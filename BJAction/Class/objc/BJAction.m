        ...
        if (!self.host)
        {
            ...
            if (query) {
                NSString *jsonString = nil;
                    ...
                    jsonString = [query stringByRemovingPercentEncoding];
#else
                    jsonString = [query stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
#endif
                
                NSError *error;
                param = [NSJSONSerialization JSONObjectWithData: [jsonString dataUsingEncoding:NSUTF8StringEncoding]
            ...
            if (!self.path || ([[[url path] substringFromIndex:1] isEqualToString:self.path])) {
                ...
                NSError *error;
                NSDictionary *param = [NSJSONSerialization JSONObjectWithData: [jsonString dataUsingEncoding:NSUTF8StringEncoding]
                                                        options: NSJSONReadingMutableContainers
                                                          error: &error];
